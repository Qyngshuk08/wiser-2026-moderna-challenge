...
% ============================================================ QAOA PENALTY BUG
\begin{frame}{8. QAOA: A Penalty-Landscape Bug, Caught and Fixed}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{cardbox}
    ...
    \begin{column}{0.5\textwidth}
      \begin{greenbox}
      \textbf{\color{accent}The fix}\\[5pt]
      \footnotesize A much tighter, QAOA-specific penalty ($\sim$1.5x the single largest quartet energy) replaces the classical-exactness penalty for QAOA runs only.
      \vspace{4pt}

      Exact match on both test sequences, confirmed with a real constraint check --- not an energy-gap proxy.
      \vspace{4pt}

      Later hardened further with post-selection: rank sampled bitstrings by shot count, return the highest-count one that is actually feasible.
      \end{greenbox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{13. Limitations}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      \begin{cardbox}
      \textbf{\footnotesize\color{badred}QAOA struggles on the hairpin-aware Hamiltonian}\\[3pt]
      \tiny Confirmed via simulated annealing that the model is correct --- the landscape is genuinely harder for a shallow circuit.
      \end{cardbox}
      \vspace{4pt}
      \begin{cardbox}
      ...
      \vspace{4pt}
      \begin{cardbox}
      \textbf{\footnotesize\color{badred}IBM access permanently exhausted}\\[3pt]
      \tiny No further hardware reruns possible under this project's access.
      \end{cardbox}
    \end{column}
  \end{columns}
\end{frame}
